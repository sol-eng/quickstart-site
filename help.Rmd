...
Documentation for all RStudio products is available from [our website](http://docs.rstudio.com/) and can also be accessed within the products in the QuickStart.

### Sample Webmail Server
  
A [sample webmail server](webmail/) is available within the QuickStart to demonstrate RStudio Connect’s capability for sending automated, customized emails.

In the QuickStart, all email from RStudio products is configured to be sent into a single mailbox. In an real-world installation, RStudio Products integrate securely with your email infrastructure to ensure that information remains private.
